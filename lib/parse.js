...
var extractMessagesFromFile = function extractMessagesFromFile(file) {
  ...
  if (file.endsWith('.vue')) {
    var _parseComponent$descr = (0, _compilerSfc.parse)(_fs["default"].readFileSync(file, 'utf8')).descriptor,
      template = _parseComponent$descr.template,
      script = _parseComponent$descr.script,
      scriptSetup = _parseComponent$descr.scriptSetup;
    var templateScript = '';
    if (template) {
      var _compileTemplate = (0, _compilerSfc.compileTemplate)({
        ...
        code = _compileTemplate.code;
      templateScript = code.replace(/^with\(this\)/, 'function _$FOOBARBAZ$_()');
    }
    var scriptTemplate = script || scriptSetup || {};
    var scripts = templateScript + '\n' + (scriptTemplate.content || '');
    return extractMessages(scripts, _objectSpread({}, opts, {
      filename: file,
